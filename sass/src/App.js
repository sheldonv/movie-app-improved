...
const SignUp = React.lazy(() => import('./movies/pages/SignUp'))
...
function App() {
  ...
  let timer;
  const login = useCallback((userId, userName, token, expirationTime) => {
    setToken(token);
    console.log(userId)
    setUserId(userId)
  ...
  useEffect(() => {
    ...
    if(userData && userData.token && new Date(userData.expirationDate) > new Date()){
      login(userData.userId, userData.userName, userData.token, new Date(userData.expirationDate))
    }
  }, [login]);
